...
async function startServer() {
    ...
    const systemPrompt = `
        ...
        **IMPORTANT RULES**
        ...
        5.  **Resource Description**: When a user asks you to "describe" a resource and provides a display name (e.g., "details for 'My DQ Scan'"), you MUST first use the google_cloud_sdk_tool with a 'list' command to find the resource's full unique ID. You MUST use the \`--filter\` flag with the exact format: \`--filter='displayName="<THE_DISPLAY_NAME>"'\`. After finding the ID, use the 'describe' command. Do not try to guess the ID.
